...
export default function SettingsPage() {
  ...
  const usingEnvKey = settings?.binanceApiKey === "__env__";
  const usingEnvSecret = settings?.binanceApiSecret === "__env__";
  const credentialsFromEnv = usingEnvKey || usingEnvSecret;
  ...
  const form = useForm<SettingsFormValues>({
    ...
    values: settings ? {
      binanceApiKey: usingEnvKey ? "" : (settings.binanceApiKey || ""),
      binanceApiSecret: usingEnvSecret ? "" : (settings.binanceApiSecret || ""),
  ...
  const testConnectionMutation = useMutation<{ success: boolean; latencyMs?: number; message?: string }, Error>({
    mutationFn: async () => {
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "الاتصال ناجح",
        description: data?.latencyMs
          ? `زمن الاستجابة التقريبي ${data.latencyMs} مللي ثانية`
          : "تم الاتصال بـ Binance API بنجاح",
      });
    },
    onError: (error) => {
      ...
      if (error instanceof Error) {
        ...
        const raw = parts.length > 1 ? parts.slice(1).join(':').trim() : error.message;
        try {
      ...
      toast({
        title: "فشل الاتصال",
        description: message && message.length > 0 ? message : "تأكد من صحة مفاتيح API",
        variant: "destructive",
      });
    },
  });
                  ...
                  {credentialsFromEnv && (
                    <div className="mt-2">
                      <Badge variant="outline" className="bg-muted/60 text-muted-foreground">
                        يتم تحميل المفاتيح من أسرار البيئة (Cloudflare)
                      </Badge>
                    </div>
                  )}
                </CardHeader>
                      ...
                      <FormItem>
                        ...
                        {usingEnvKey && (
                          <FormDescription className="text-xs text-muted-foreground">
                            يتم استخدام مفتاح محفوظ كسِر بيئي. اكتب قيمة جديدة إذا أردت استبداله.
                          </FormDescription>
                        )}
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        {usingEnvSecret && (
                          <FormDescription className="text-xs text-muted-foreground">
                            يتم استخدام السر من البيئة. أدخل قيمة جديدة لاستبداله في التخزين المحلي.
                          </FormDescription>
                        )}
                        <FormMessage />
                      </FormItem>
